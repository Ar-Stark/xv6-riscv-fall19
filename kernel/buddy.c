...
//增加操作alloc数组的函数，因为在alloc数组中两个block共用一个bit
// 翻转 bit at position index, 1->0, 0->1 
void bit_convert(char *array, int index){
  index = index/2;
  char b = array[index/8];
  char m = (1 << (index % 8));
  array[index/8] = (b ^ m);//异或
}


//如果bit at position index in array为1，返回1，代表一对buddy中一个free，一个allocated
int bit_allocisset(char *array, int index){
  index = index/2;
  char b = array[index/8];
  char m = (1 << (index % 8));
  return (b & m) == m;
...
bd_malloc(uint64 nbytes)
{
  ...
  bit_convert(bd_sizes[k].alloc, blk_index(k, p));
  for(; k > fk; k--) {
    ...
    bit_convert(bd_sizes[k-1].alloc, blk_index(k-1, p));
...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_convert(bd_sizes[k].alloc, bi);
    //if (bit_isset(bd_sizes[k].alloc, buddy)) {  // is buddy allocated?
    if (bit_allocisset(bd_sizes[k].alloc, bi)){
...
bd_initfree_pair(int k, int bi, void* left, void* right) {
  ...
  if(bit_allocisset(bd_sizes[k].alloc, bi)){
    ...
    if(addr_in_range(addr(k, bi), left, right, free))//bi在left和right之间，说明bi是free的
